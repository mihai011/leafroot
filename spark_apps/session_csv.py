...
def main() -> None:
    ...
    # reading directly into a DataFrame with SparSession,
    # but could be json, parquet, text etc.
    ...
    person_df = spark.read.csv(
        "file:///opt/spark-data/person.csv",
        header=True,
    ...
    window_dept = Window.partitionBy("Department_Name").orderBy(col("Salary").desc())
    joined_df = (
        person_df.join(department_df, "Department_ID")
        .select(["Department_Name", "Person_Name", "Salary"])
        .withColumn("position_in_department", row_number().over(window_dept))
    ...
    joined_df = spark.sql(
        """
                ...
                (select *,row_number() OVER (PARTITION BY Department_Name
                ORDER BY salary DESC) as position_in_department
                from Person p join Department d on d.Department_ID = p.Department_ID)
                where position_in_department = 1""",
    )
    print(joined_df.show())
    print(joined_df.printSchema())
