...
echo "CREATE DATABASE ${AIRFLOW_DB};" | PGPASSWORD=$POSTGRES_PASSWORD psql --user $POSTGRES_USER --host $POSTGRES_HOST
echo "CREATE USER ${AIRFLOW_DB_USER} WITH SUPERUSER ENCRYPTED PASSWORD '${AIRFLOW_DB_PASS}'" | PGPASSWORD=$POSTGRES_PASSWORD psql --user $POSTGRES_USER --host $POSTGRES_HOST
echo "GRANT ALL ON SCHEMA public TO ${AIRFLOW_DB_USER};" | PGPASSWORD=$POSTGRES_PASSWORD psql --user $POSTGRES_USER --host $POSTGRES_HOST
echo "GRANT ALL PRIVILEGES ON DATABASE ${AIRFLOW_DB} TO ${AIRFLOW_DB_USER}" | PGPASSWORD=$POSTGRES_PASSWORD psql --user $POSTGRES_USER --host $POSTGRES_HOST
